...
def batchify(fn, chunk=1024 * 32, axis=0):
    ...
    def ret_func(**kwargs):
        ...
        for i in range(0, x.shape[axis], chunk):
            ...
            chunk_kwargs = dict(
                [
                    [key, torch.index_select(kwargs[key], axis, torch.arange(i, end).to(kwargs[key].device))]
...
def p2f_dist_bachify(pfc_verts, pfc_faces, points, batch_size=64 * 64):
    ...
    while start < pfc_verts.shape[0]:
        ...
        offset = torch.linspace(
            0, (idx_.shape[0] - 1) * pfc_faces.shape[1], idx_.shape[0], dtype=torch.long, device=idx_.device
        )
        idx_ = idx_ - offset
        idx[start:end] = idx_
        start = end
...
def sample_patches_from_GelSight(
    patch_size=128,
    max_patch_num=2000,
    scan_pcl_path=None,
    ...
    """
    This function is modified from the function "sample_patches"
    It convert a batch of GelSight Mini data to a stack of patches for 3D texture synthesis.

    Requirement for GelSight input data:
    for each frame, we need
    1. a 320x240 (img_w, img_h) depth image/array
    2. a vertice (x, y, z) representing the global position of this patch on the base shape, which we assume to be a flat surface for now
    """
    ...
    # TODO: for GelSight data, we provide the samples as a stack of GelSight sensor data, so we don't need the mesh_for_sample
    # Sample on picked faces or the whole mesh

    # Initialize patch coordinates
    grid_gap = 1.0
    calibration = np.linspace(
        -patch_size * grid_gap / 2, patch_size * grid_gap / 2, patch_size
    )  # create a 1D grid with size patch_size, grid distance grid_gap, e.g. patch_size = 2, grid_gap 0.1, patch_size refers to how many grids.
    ...
    patch_phi_embed = None
    patch_local_tbn = np.zeros([max_patch_num, patch_size, patch_size, 9])
    patch_rays = np.zeros([max_patch_num, patch_size, patch_size, 6])
    count = 0

    gs_images = np.zeros((200, 320, 240))  # num_images, img_w, img_h
    img_w = gs_images.shape[1]
    img_h = gs_images.shape[2]
    vertices = np.zeros((200, 3))
    ...
    # sample one patch as each GelSight image, and assign a psuedo "vertice" on the plane mesh to each patch
    for i in tqdm(range(len(gs_data["gs_images"]))):
        # for each tangent plane of the coarse mesh
        # i.e., for each GelSight mini image
        # we put a square scan rays of 128x128 resolution
        # TODO: set the "simulated vertices coord for each GelSight patch"

        # temporarily center crop 128x128 from the GelSight image
        # to simulate the 128x128 resolution scan rays
        start_h = (img_h - patch_size) // 2
        start_w = (img_w - patch_size) // 2
        gs_images[i] = gs_images[i][start_h : start_h + patch_size, start_w : start_w + patch_size]

        # Determine the transform matrix by sample vertex. (Done)
        # For GelSight data, assume the base shape is a flat surface for now
        z_axis = np.array(
            [0, 0, 1]
        )  # v_normals[i], assume normals of the base shape of each point on GelSight is flat.
        first_component = np.array(
            [1, 0, 0]
        )  # v_tangents[i], assume tangents of the base shape of each point on GelSight is flat.
        ...
        T[:3, 3] = gs_data["vertices"][i]  # different vertices corresponds to different locations on the base shape

        # Find the intersections of the square scan rays and the coarse mesh (Done)
        ...
        ray_directions = torch.from_numpy(np.copy(np.broadcast_to(-z_axis[None], ray_origins.shape))).float().to(device)

        # TODO: if the intersections are intersection between scan rays with coarse mesh, then the following implementation is correct;
        # otherwise, if the intersections are intersection between scan rays with fine mesh, then need to set up a raytracer to find the intersections.
        intersections = (
            0.0 * ray_origins
        )  # For GelSight data, the square is tangent to the plane coarse shape, so the square lies on the plane and the depth are all zeros.

        # Gater patches
        # RH: my understanding is: intersections are the query points x in Fig. 4
        # p_sur, _, _, normal, local_tbn = self.meshprojector.project(intersections, K=self.K, h_threshold=h_threshold, requires_grad_xyz=False)

        # RH: my understanding is:
        # p_sur - point projected on the surface, x_c in Fig. 4
        # normal - coarse normal vector at x_c, n_c in Fig. 4
        # local_tbn - local tangent space matrix at x_c, for GelSight, should be facing upward, i.e. positive z axis
...
class Factorized_Normal_Net(nn.Module):
    def __init__(
        ...
        x_dim,
        z_dim,
        theta_scale=np.pi / 2 * 1.1,
        phi_scale=np.pi * 2 * 1.1,
        bound_output=False,
        lip=True,
        low_freq_band_len_f=32,
        low_freq_band_len_z=12,
        direct_pred_coor=False,
    ):
        super().__init__()
        ...
        if direct_pred_coor:
            if lip:
                self.coor_net = LipMLP(
                    in_dim=self.encoder_out_dim + self.low_freq_band_len_z, out_dim=3, n_neurons=32, num_layers=3
                )
            else:
            ...
            if lip:
                self.phi_net = LipMLP(
                    in_dim=self.encoder_out_dim + self.low_freq_band_len_z, out_dim=1, n_neurons=16, num_layers=2
                )
                # Predict theta based on geometry features
                self.theta_net = LipMLP(
                    in_dim=self.low_freq_band_len_x + self.low_freq_band_len_z, out_dim=1, n_neurons=16, num_layers=2
                )
            else:
    ...
    def forward(self, z_embed, x_embed, p_sur=None, phi_embed=None, tbn=None, return_rot_angles=False):
        ...
        if self.direct_pred_coor:
            ...
            geo_feat = torch.cat(
                [x_embed[..., : self.low_freq_band_len_x], z_embed[..., : self.low_freq_band_len_z]], dim=-1
            )
...
class MeshProjector:
    def __init__(
        ...
        if store_uv:
            ...
            self.uvs = (uvs - uvs.min()) / (uvs.max() - uvs.min()) * 2 - 1.0  # Map to -1~1
        else:
            self.uvs = None

        # Calculate local Tangent, Bitangent, Normal coordinate system
        self.tbn = (
            torch.from_numpy(calculate_tbn(self.mesh, self.uvs.cpu().numpy())).float().to(device)
    ...
    def weighted_project(
        ...
        sdf_offset=0.0,
        dir_vec_wdist=0.05,
        direct_above_check=False,
        direct_above_threshold=1e-1,
    ):
        normal, dir_vec, indices, dis = self.knn(
            xyz=xyz,
            K=K,
            use_dir_vec=False,
            dir_vec_wdist=dir_vec_wdist,
    ...
    def knn(
        ...
        if use_dir_vec:
            ...
            mean_dir_vec = torch.where(
                (mean_dir_vec * normal_test).sum(dim=-1, keepdims=True) < 0, -mean_dir_vec, mean_dir_vec
            )
    ...
    def query_tbn(self, xyz, K=8, h_threshold=None, sdf_scale=1.0, sdf_offset=0.0):
        normal, _, _, _ = self.knn(xyz=xyz, K=K, use_dir_vec=False, weighting="DualD", nn_consis_check=True)
        _, _, _, h_mask, face_idx = self.barycentric_mapping(
            xyz=xyz,
            normal=normal,
    ...
    def uvh(self, xyz, K=8, h_threshold=None, sdf_scale=1.0, sdf_offset=0.0, requires_grad_xyz=False, normal=None):
        if normal is None:
            normal, _, _, _ = self.knn(xyz=xyz, K=K, use_dir_vec=False, weighting="DualD", nn_consis_check=True)
        vertex_idx, barycentric, sdf, h_mask, face_idx = self.barycentric_mapping(
            xyz=xyz,
            normal=normal,
            h_threshold=h_threshold,
            sdf_scale=sdf_scale,
...
class MeshFeatureField(nn.Module):
    def __init__(
        ...
        if hash:
            ...
            self.encoder, self.encoder_f_out_dim = get_encoder(
                grid_name,
                desired_resolution=1024,
                input_dim=3,
            ...
            if self.prob_model:
                self.encoder_var, self.encoder_f_out_dim_var = get_encoder(
                    "hashgrid",
                    desired_resolution=1024,
                    input_dim=3,
                    num_levels=num_level,
            ...
            self.levels_vnum = (
                np.stack(
                    [base_vnum * 2**i for i in np.linspace(0, np.log2(self.target_vnum / base_vnum), self.level_num)]
                )
                if self.level_num > 1
                else np.array([self.target_vnum])
            )
        ...
        if pred_normal:
            self.normal_net = Factorized_Normal_Net(
                x_dim=self.encoder_f_out_dim,
                z_dim=self.encoder_z_outdim,
                lip=self.use_lip_mlp_for_normal,
                direct_pred_coor=False,
        ...
        self.sdf_scale_factor = 1.0
        self.sdf_offset = 0.0
        self.uv_utilize_rate = 1.0
        self.K_for_uv = 5
    ...
    def forward(
        ...
        if not self.imported or no_import or self.imported_type is None:
            # No import mode
            p_sur, sdf, h_mask, normal_coarse, local_tbn = self.meshprojector.project(
                x, K=self.K, h_threshold=self.h_threshold, requires_grad_xyz=requires_grad_xyz
            )
            if self.hash:
                # Embed surface coordinates with hash grid
                ...
                vertex_idx, barycentric, sdf, h_mask = self.meshprojector_fea.barycentric_mapping(
                    x, normal_coarse, h_threshold=self.h_threshold, requires_grad_xyz=requires_grad_xyz
                )
            ...
            if self.imported_type == "field":
                ...
                h_mask = torch.logical_and(h_mask, (p_sur[..., :2] >= -1.0).all(dim=-1))
                h_mask = torch.logical_and(h_mask, (p_sur[..., :2] <= 1.0).all(dim=-1))
                x_embed = (
                    torch.nn.functional.grid_sample(
                        self.features_imported, p_sur[None, None, :, :2], align_corners=True, padding_mode="zeros"
                    )
                    .squeeze()
                    .permute(1, 0)
                )
                ...
                if self.pred_normal:
                    ...
                    phi_embed = (
                        torch.nn.functional.grid_sample(
                            self.phi_embed_imported, p_sur[None, None, :, :2], align_corners=True, padding_mode="zeros"
                        )
                        .squeeze()
                        .permute(1, 0)
                    )
                ...
                # Patch resampled
                sdf, idx, weights, normal_coarse, dis = self.meshprojector_imported.weighted_project(
                    x, K=8, direct_above_check=True, direct_above_threshold=1.0
                )
                ...
                # Shape
                uvh, h_mask, normal_coarse, new_tbn = self.meshprojector_imported.uvh(
                    x,
                    K=self.K_for_uv,
                ...
                vertex_idx, barycentric, sdf, h_mask = self.meshprojector_imported.barycentric_mapping(
                    x,
    ...
    def kl_loss(self, normal=False):
        if self.prob_model:
            ...
            if normal:
                kl_loss = 0.5 * (torch.exp(f_var) + f_mu**2 - 1.0 - f_var).sum()
            else:
                kl_loss = 0.5 * (torch.exp(f_var) - 1.0 - f_var).sum()
            return kl_loss
        else:
            return 0.0
    ...
    def uv(self, x):
        ...
        if meshprojector.uvs is not None:
            sdf_scale_factor = self.sdf_scale_factor if self.imported_type == "shape" else 1.0
            sdf_offset = self.sdf_offset if self.imported_type == "shape" else 0.0
            uvh, h_mask, _, _ = meshprojector.uvh(
                x, K=self.K_for_uv, h_threshold=self.h_threshold, sdf_scale=sdf_scale_factor, sdf_offset=sdf_offset
            )
        else:
    ...
    def update(self, target_level=None):
        ...
        if self.current_level < self.level_num - 1:
            ...
            fea_mesh_path = self.subdivide_mesh(
                min_vnum=self.levels_vnum[self.current_level],
                obj_suffix=f"fea_level_{self.current_level}",
                replace=True,
            )
            fea_mesh = trimesh.load_mesh(fea_mesh_path)
            features = torch.empty(
                (fea_mesh.vertices.shape[0], self.feature_dim), dtype=torch.float32, device=self.device
            )
            ...
            if self.current_level == 0:
                ...
                while start < vertices.shape[0]:
                    ...
                    _, _, _, normal, _ = self.meshprojector.project(
                        vertices[start:end], K=self.K, h_threshold=self.h_threshold
                    )
                    vertex_idx, barycentric, _, _ = self.meshprojector_fea.barycentric_mapping(
                        vertices[start:end], normal, h_threshold=self.h_threshold
                    )
    ...
    def import_unhash_vertices(self, data_path):
        ...
        self.meshprojector_imported = BvhMeshProjector(
            device=self.device, mesh=data["mesh"][()], store_f=True, compute_normals=True, ini_raytracer=True
        )
    ...
    def import_unhash(self, data_path, res=2048):
        ...
        features_imported = (
            torch.zeros([res, res, features.shape[-1]], dtype=torch.float32)
        ...
        meshprojector_plane = BvhMeshProjector(
            device=self.device, mesh=mesh_plane, store_f=True, compute_normals=True, ini_raytracer=True
        )
        # Features-Vertices Mesh
        meshprojector_complex = BvhMeshProjector(
            device=self.device, mesh=data["mesh"][()], store_f=True, compute_normals=True, ini_raytracer=True
        )
        # Traverse UV map
        ...
        while start < uvs.shape[0]:
            ...
            sdf, fids, barycentric = meshprojector_plane.bvh.signed_distance(
                uvs[start:end], return_uvw=True, mode="raystab"
            )
            verts_3d = meshprojector_plane.barycentric_weighting(
                vert_values=self.meshprojector_imported.mesh_vertices, fids=fids, barycentric=barycentric
            )
        ...
        self.sdf_scale_factor = float(data["sdf_factor"])
        self.bounds = [0.5 * data["original_grid_gap"] * res, 0.5 * data["original_grid_gap"] * res]
        self.rescale = True
        print("Unhashed Synthesized Results! sdf factor: ", self.sdf_scale_factor)
        torch.cuda.empty_cache()
    ...
    def import_field(self, features, bounds, sample_tbn, sample_tbn_ids, local_tbn, phi_embed):
        ...
        self.features_imported = (
            torch.from_numpy(features).permute(0, 1, 3, 2).to(self.device).float()
        )  # Permute to let uv correspond to xy
        self.phi_embed_imported = torch.from_numpy(phi_embed).permute(0, 1, 3, 2).to(self.device).float()
        self.sample_tbn_inv_imported = (
            torch.from_numpy(sample_tbn).to(self.device).float().reshape([-1, 3, 3]).inverse()
        )
    ...
    def import_patch(self, features, mesh, local_tbn, phi_embed):
        self.vnum = features.shape[0]
        self.meshprojector_imported = MeshProjector(
            device=self.device,
            mesh=mesh,
    ...
    def import_shape(self, mesh):
        if not (self.imported_type == "field" or self.imported_type == "shape"):
            ...
            return
        self.meshprojector_imported = MeshProjector(
            device=self.device, mesh=mesh, compute_normals=True, ini_raytracer=True, store_f=True, store_uv=True
        )
    ...
    def sample_patches(
        ...
        print(
            f"In function sample_patches, mean_edge_length: {mean_edge_length}, pattern_rate {self.pattern_rate}, patch_size: {patch_size}, patch_edge_length: {patch_edge_length}"
        )
        ...
        if sample_on_template:
            ...
            vertices, face_idx = trimesh.sample.sample_surface_even(
                mesh_for_sample, 20000, radius=patch_edge_length / 16
            )
        ...
        picked_vertices = np.zeros([max_patch_num, 3])
        patch_phi_embed = (
            np.zeros([max_patch_num, patch_size, patch_size, self.normal_net.encoder_out_dim])
            if self.normal_net is not None
            else None
        )
        patch_local_tbn = np.zeros([max_patch_num, patch_size, patch_size, 9])
        patch_rays = np.zeros([max_patch_num, patch_size, patch_size, 6])
        ...
        for i in tqdm(range(vertices.shape[0])):
            ...
            if use_trimesh_raycast:
                ...
                if nn_dist.max() > min(1e-1, 3 * self.h_threshold):
                    continue
                ray_origins += 0.1 * torch.from_numpy(z_axis).float().to(self.device)
                ray_directions = (
                    torch.from_numpy(np.copy(np.broadcast_to(-z_axis[None], ray_origins.shape))).float().to(self.device)
                )
            ...
            # Gater patches
            p_sur, _, _, normal, local_tbn = self.meshprojector.project(
                intersections, K=self.K, h_threshold=self.h_threshold, requires_grad_xyz=False
            )
            if self.hash:
                # Embed surface coordinates with hash grid
                ...
                # Embed surface points with barycentric features weighting
                vertex_idx, barycentric, _, _ = self.meshprojector_fea.barycentric_mapping(
                    intersections, normal, h_threshold=self.h_threshold, requires_grad_xyz=False
                )
    ...
    def visualize_features(self, sv_path="./"):
        ...
        for i in range(self.encoder.num_levels):
            ...
            plt.scatter(
                cluster_centers[:, 0], cluster_centers[:, 1], c=np.arange(cluster_centers.shape[0]), cmap="viridis"
            )
